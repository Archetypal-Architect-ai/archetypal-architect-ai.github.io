...
const missing=[];
for(const file of files){const html=fs.readFileSync(path.join(root,file),'utf8').replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi,'');for(const m of html.matchAll(/(?:href|src)="(\/[^"#?]*)(?:[?#][^"]*)?"/g)){const dest=path.join(root,decodeURI(m[1]));if(!fs.existsSync(dest)&&!fs.existsSync(path.join(dest,'index.html')))missing.push([file,m[1]]);}}
...
assert(read('novels').includes('https://a.co/d/0ceG8cHV'));
...
console.log(`Checked ${files.length} pages: internal routes/assets resolve; book, shop, social, legacy and playback markup assertions pass.`);
